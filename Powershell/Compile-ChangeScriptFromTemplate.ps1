...
function Compile-ChangeScriptFromTemplate
{
  ...
  try{
    ...
    Write-Error "`nChange script compiled version prior to error:"
    Write-Error $localResult
    throw
  }
}
...
function Get-TokenHash {
  ...
  try{
    ...
    Write-Error "`nGetting Tokens and Values failed`n"
    Write-Error $_.Exception

    Write-Error "Tokens:"
    $tokenHash.Keys | Where-Object {$_ -ne $null} | % {Write-Host $_}

    Write-Error "Token Values:"
...
function Get-SectionHash {
  ...
  try {
    ...
    Write-Error "`nGetting Section Tokens and Values failed`n"
    Write-Error $_.Exception

    Write-Error "Sections:"
    $sectionHash.Keys | Where-Object {$_ -ne $null} | % {Write-Host $_}

    Write-Error "Section Values:"
